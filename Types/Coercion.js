...
function hoursAttended(attended, length) {
  if (typeof attended === "string" && attended.trim() !== "") attended = Number(attended);
  if (typeof length === "string" && length.trim() !== "") length = Number(length);

  if (typeof attended !== "number" && typeof length !== "number") return false;
  if (Math.min(attended, length) < 0) return false;
